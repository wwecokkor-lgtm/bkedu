...
import type { NewsPost, Notice, Comment } from './types';
...
export const contentApi = {
    ...
    getNewsPosts: async (options: { page?: number, limit?: number, category?: string, tag?: string, sort?: 'date' | 'views', status?: string } = {}, isPreview: boolean = false): Promise<{ posts: NewsPost[], total: number }> => {
        ...
        // If isPreview is true and no specific status is requested, show all (draft, scheduled, published).
    ...
    updateNewsPost: async (adminId: string, postId: string, postData: Partial<NewsPost>): Promise<{ success: boolean; post?: NewsPost; message: string }> => {
        ...
        mockNewsPosts[postIndex] = { ...originalPost, ...postData, slug: postData.title ? generateSlug(postData.title) : originalPost.slug };
        
        logAdminAction(adminId, `Updated post: ${mockNewsPosts[postIndex].title}`, 'NewsPost', postId);
        return { success: true, post: mockNewsPosts[postIndex], message: 'Post updated successfully!' };
    },
