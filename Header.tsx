...
const Header: React.FC<HeaderProps> = ({ logoUrl }) => {
    const { isAuthenticated, user, logout, page, setPage, notifications, searchTerm, setSearchTerm } = useAppStore();
            ...
            <nav className="container mx-auto px-4">
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center">
                        <div className="flex-shrink-0 text-white font-bold text-xl cursor-pointer" onClick={() => setPage('dashboard')}>
                             {logoUrl ? <img src={logoUrl} alt="BK Academy" className="h-8 w-auto" /> : 'BK Academy'}
                        </div>
                        {isAuthenticated && (
                            <div className="hidden md:block ml-10">
                                <div className="flex items-baseline space-x-4">
                                    <NavLink onClick={() => setPage('dashboard')} isActive={page === 'dashboard'}>Dashboard</NavLink>
                                    <NavLink onClick={() => setPage('courses')} isActive={page === 'courses'}>Our Courses</NavLink>
                                    <NavLink onClick={() => setPage('exams')} isActive={page === 'exams'}>Exams</NavLink>
                                    <NavLink onClick={() => setPage('leaderboard')} isActive={page === 'leaderboard'}>Leaderboard</NavLink>
                                    <NavLink onClick={() => setPage('news')} isActive={page === 'news' || page === 'newsDetail'}>News</NavLink>
                                    {(user?.role === Role.ADMIN || user?.role === Role.SUPER_ADMIN) && <NavLink onClick={() => setPage('admin')} isActive={page === 'admin'}>Admin Panel</NavLink>}
                                </div>
                            </div>
                        )}
                    </div>
                            ...
                            <div className="relative" ref={dropdownRef}>
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute right-0 mt-2 w-48 bg-slate-800 rounded-md shadow-lg py-1 border border-slate-700">
                                        ...
                                        <button onClick={() => { setPage('dashboard'); setIsDropdownOpen(false); }} className="w-full text-left block px-4 py-2 text-sm text-slate-300 hover:bg-slate-700">Dashboard</button>
                                        <button onClick={() => { logout(); setIsDropdownOpen(false); }} className="w-full text-left block px-4 py-2 text-sm text-slate-300 hover:bg-slate-700 border-t border-slate-700">Logout</button>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {isAuthenticated && isMobileMenuOpen && (
                <div ref={mobileMenuRef} className="md:hidden bg-slate-800 border-b border-slate-700">
                    <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                        <NavLink onClick={() => { setPage('dashboard'); setIsMobileMenuOpen(false); }} isActive={page === 'dashboard'}>Dashboard</NavLink>
                        <NavLink onClick={() => { setPage('courses'); setIsMobileMenuOpen(false); }} isActive={page === 'courses'}>Our Courses</NavLink>
                        <NavLink onClick={() => { setPage('exams'); setIsMobileMenuOpen(false); }} isActive={page === 'exams'}>Exams</NavLink>
                        <NavLink onClick={() => { setPage('leaderboard'); setIsMobileMenuOpen(false); }} isActive={page === 'leaderboard'}>Leaderboard</NavLink>
                        <NavLink onClick={() => { setPage('news'); setIsMobileMenuOpen(false); }} isActive={page === 'news' || page === 'newsDetail'}>News</NavLink>
                        {(user?.role === Role.ADMIN || user?.role === Role.SUPER_ADMIN) && <NavLink onClick={() => { setPage('admin'); setIsMobileMenuOpen(false); }} isActive={page === 'admin'}>Admin Panel</NavLink>}
                        ...
                        <button onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="w-full text-left block px-3 py-2 rounded-md text-sm font-medium text-slate-300 hover:bg-slate-700 hover:text-white">Logout</button>
                    </div>
                </div>
            )}
        </header>
