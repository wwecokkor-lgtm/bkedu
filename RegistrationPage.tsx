...
const RegistrationPage: React.FC = () => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file && file.type.startsWith('image/') && file.size <= 2 * 1024 * 1024) {
            ...
            addNotification('Please select an image file smaller than 2MB.', NotificationType.ERROR);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const registrationData = { ...formData, avatarUrl: uploadedAvatarUrl };
        const response = await api.register(registrationData);
        setIsLoading(false);

        if (response.success && response.user) {
            addNotification(response.message, NotificationType.SUCCESS);
            login(response.user); // Auto-login the user
        } else {
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <p className="text-slate-400">
                            ...
                            <button onClick={() => setPage('login')} className="font-semibold text-sky-500 hover:text-sky-400">
                                Login
                            </button>
                        </p>
                    </div>
