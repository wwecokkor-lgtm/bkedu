            ...
            <main className="flex-1 flex flex-col overflow-y-auto">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <Card><h2 className="text-xl font-bold mb-4">Comments ({activeLesson.comments.length})</h2><div className="space-y-4">
                        <div className="flex items-start gap-3"><img src={user?.avatarUrl} alt="you" className="w-9 h-9 rounded-full" /><div className="flex-grow space-y-2"><textarea value={commentText} onChange={e => setCommentText(e.target.value)} placeholder="Ask a question or leave a comment..." rows={3} className="w-full bg-slate-700 border-slate-600 rounded-md p-2 text-sm" /> <Button onClick={handleCommentSubmit} isLoading={isSubmittingComment} size="sm">Post</Button></div></div>
                    ...
                    {course.lessons.map((lesson, index) => {
                        ...
                        return (
                            <li key={lesson.id}><button onClick={() => handleLessonClick(lesson.id)} className={`w-full text-left p-3 rounded-md flex items-start gap-3 ${selectedLessonId === lesson.id ? 'bg-sky-600/20' : 'hover:bg-slate-700/50'}`}>
                                <div className={`mt-1 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${isLessonCompleted ? 'bg-green-500' : 'border-2 border-slate-500'}`}>{isLessonCompleted && <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}</div>
